                ...
                case CompressionType.Lz4HC:
                    {
                        ...
                        try
                        {
                            ...
                            {
                                throw new IOException($"LZ4解压缩失败,实际写入{numWrite}字节但预期{uncompressedSize}字节");
                            }
        ...
        private void ReadBlocks(FileReader reader, Stream blocksStream)
        {
            ...
            for (int i = 0; i < m_BlocksInfo.Count; i++)
            {
                ...
                switch (compressionType) //kStorageBlockCompressionTypeMask
                {
                    ...
                    case CompressionType.OodleMr0k:
                        {
                            ...
                            try
                            {
                                ...
                                if (numWrite != uncompressedSize)
                                {
                                    Logger.Warning($"Oodle解压缩错误:实际写入{numWrite} 字节，预期{uncompressedSize}字节");
                                }
                            }
                            finally
                            {
                    ...
                    case CompressionType.Lz4HC:
                    case CompressionType.Lz4Mr0k when Game.Type.IsMhyGroup():
                        {
                            var compressedSize = (int)blockInfo.compressedSize;
                            var uncompressedSize = (int)blockInfo.uncompressedSize;
                            ...
                            try
                            {
                                ...
                                if (numWrite != uncompressedSize)
                                {
                                    throw new IOException($"LZ4解压缩错误,实际写入{numWrite}字节,但预期为{uncompressedSize}字节");
                                }
                                blocksStream.Write(uncompressedBytesSpan);
                            }
                            finally
                    ...
                    case CompressionType.Zstd when !Game.Type.IsMhyGroup(): //Zstd
                        {
                            ...
                            try
                            {
                                ...
                                if (numWrite != uncompressedSize)
                                {
                                    throw new IOException($"Zstd解压出错,实际写入{numWrite}字节,但预期为{uncompressedSize}字节");
                                }
