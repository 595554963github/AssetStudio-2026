...
namespace AssetStudio
{
    public static class ImportHelper
    {
        ...
        {
            Logger.Verbose($"在加载文件之前处理拆分资源(.splitX)...");
            ...
            if (startIdx != -1 && startIdx != paths.Length - 1)
            {
                ...
                if (offset > 0 && offset < reader.Length)
                {
                    ...
                    if (signature == "UnityFS")
                    {
                        ...
                        Logger.Verbose("解析ImaginaryFest成功!!");
                        reader.Position = offset;
        ...
        {
            Logger.Verbose($"尝试去解密Reverse1999加密的文件{reader.FileName}");
            ...
            if (signature == "UnityFS")
            {
                ...
                Logger.Verbose("解密Reverse1999成功!!");
        ...
        {
            Logger.Verbose($"尝试解密咒术回战:幻影游行的加密文件{reader.FileName}");
        ...
        {
            Logger.Verbose($"正在尝试解密恋与深空的文件{reader.FileName}");
        ...
        {
            Logger.Verbose($"尝试解密新月同行文件{reader.FileName}");
        ...
        {
            Logger.Verbose($"尝试去解密火影忍者的文件{reader.FileName}");
            ...
            {
                Logger.Verbose($"未知的魔数签名{magic},终止操作...");
                reader.Position = 0;
                return reader;
            }
            reader.Position -= 1;
