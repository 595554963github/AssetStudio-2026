    ...
    public class SerializedFile
    {
        ...
        public SerializedFile(FileReader reader, AssetsManager assetsManager)
        {
            ...
            if (header.m_Version >= SerializedFileFormatVersion.Unknown_7)
            {
                ...
                Logger.Verbose($"Unity版本{unityVersion}");
            ...
            if (header.m_Version >= SerializedFileFormatVersion.Unknown_8)
            {
                ...
                {
                    Logger.Verbose($"解析目标格式{m_TargetPlatform}不匹配任何支持的格式,默认{BuildTarget.UnknownPlatform}");
                    m_TargetPlatform = BuildTarget.UnknownPlatform;
                }
                else if (m_TargetPlatform == BuildTarget.NoTarget && game.Type.IsMhyGroup())
                {
                    Logger.Verbose($"选择的游戏{game.Name}是米哈游的游戏,强制目标格式{BuildTarget.StandaloneWindows64}");
            ...
            Logger.Verbose($"找到{typeCount}序列化类型");
            ...
            Logger.Verbose($"找到{objectCount}项目");
            for (int i = 0; i < objectCount; i++)
            {
                ...
                Logger.Verbose($"项目信息:{objectInfo}");
            ...
            if (header.m_Version >= SerializedFileFormatVersion.HasScriptTypeIndex)
            {
                ...
                Logger.Verbose($"找到{scriptCount}脚本");
            ...
            Logger.Verbose($"找到{externalsCount}外部的");
            for (int i = 0; i < externalsCount; i++)
            {
                ...
                Logger.Verbose($"外部的信息:{m_External}");
            ...
            if (header.m_Version >= SerializedFileFormatVersion.SupportsRefObject)
            {
                ...
                Logger.Verbose($"找到{refTypesCount}引用类型");
        ...
        {
            Logger.Verbose($"尝试解析序列化" + (isRefType ? "参考文献" : " ") + "字节");
            ...
            {
                Logger.Verbose($"编码类ID{type.classID},解码中...");
        ...
        {
            Logger.Verbose($"尝试解析blob类型树...");
            int numberOfNodes = reader.ReadInt32();
            int stringBufferSize = reader.ReadInt32();
            Logger.Verbose($"找到{numberOfNodes}节点和{stringBufferSize}字符串");
        ...
        {
            Logger.Verbose($"缓存对象{obj.m_PathID}在文件里{fileName}...");
            Objects.Add(obj);
