...
namespace AssetStudio
{
    public class AssetsManager
    {
                    ...
                    foreach (var sharedFile in assetsFile.m_Externals)
                    {
                        ...
                        if (!importFilesHash.Contains(sharedFileName))
                        {
                            ...
                            if (!noexistFiles.Contains(sharedFilePath))
                            {
                                ...
                                {
                                    Logger.Verbose("什么也没找到,缓存到不存在的文件中避免重复搜索!!");
        ...
        private void LoadZipFile(FileReader reader)
        {
            ...
            try
            {
                using (ZipArchive archive = new ZipArchive(reader.BaseStream, ZipArchiveMode.Read))
                {
                    ...
                    Logger.Verbose("在解析资源之前注册所有文件，以便可以找到外部引用并找到拆分文件");
        ...
        private void ProcessAssets()
        {
            ...
            Dictionary<string, PPtr<Mesh>> separateMeshes = null;
            List<GameObject> avatars = null;

            if (Game.Type.IsZZZGroup())
            {
                separateMeshes = new Dictionary<string, PPtr<Mesh>>();
                    ...
                    if (obj is GameObject m_GameObject)
                    {
                        ...
                        if (Game.Type.IsZZZGroup())
                        {
                            if (m_GameObject.m_Animator != null ||
                                m_GameObject.Name.StartsWith("Avatar_") ||
                                m_GameObject.Name.EndsWith("_Model"))
                            {
                                avatars.Add(m_GameObject);
                            }
                        }
                    }
                    else if (obj is Mesh mesh && Game.Type.IsZZZGroup())
                    {
                        var pptr = new PPtr<Mesh>(0, mesh.m_PathID, assetsFile);
                        if (pptr.TryGet(out var meshObj))
                        {
                            if (!separateMeshes.ContainsKey(mesh.Name))
                            {
                                separateMeshes.Add(mesh.Name, pptr);
                            }
                        }
                    }
            ...
            if (Game.Type.IsZZZGroup() && separateMeshes != null && avatars != null)
            {
                Logger.Info($"找到{separateMeshes.Count}个分离网格");
                Logger.Info($"找到{avatars.Count}个Avatar");
                    ...
                    if (avatar.m_Transform != null)
                    {
                        foreach (var childPtr in avatar.m_Transform.m_Children)
                        {
                            if (childPtr.TryGet(out var child) && child.m_GameObject.TryGet(out var childGO))
                            {
                                ...
                                {
                                    AttachMeshToGameObject(childGO, meshPPtr);
                                }
        ...
        private void AttachMeshToGameObject(GameObject go, PPtr<Mesh> meshPtr)
        {
            if (go.m_SkinnedMeshRenderer != null && go.m_SkinnedMeshRenderer.m_Mesh.IsNull)
            {
                go.m_SkinnedMeshRenderer.m_Mesh = meshPtr;
            }
            else if (go.m_MeshFilter != null && go.m_MeshFilter.m_Mesh.IsNull)
            {
                go.m_MeshFilter.m_Mesh = meshPtr;
            }
        }
    }
}